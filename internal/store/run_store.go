...
func (s *Store) bumpCounts(runID int64, steps, effs, comps, retries int) error {
	...
	_, err = s.db.Exec(
		`UPDATE runs SET step_count=?, eff_count=?, comp_count=?, retry_count=?, updated_at=? WHERE id=?`,
		run.StepCount+steps, run.EffCount+effs, run.CompCount+comps, run.RetryCount+retries, fmtTime(time.Now().UTC()), runID,
	)
	return err
}
