...
func TestVerifyRunPersistsCompGrades(t *testing.T) {
	app := newProbeApp(t)
	run, err := app.CreateRun("probe-comp-grade", "probe")
	if err != nil {
		t.Fatal(err)
	}
	...
	comps, err := app.Store.ListCompensations(run.ID)
	if err != nil {
		t.Fatal(err)
	}
	if len(comps) != 1 {
	...
	if comps[0].Status != model.CompValid {
		t.Fatalf("comp status=%q want valid", comps[0].Status)
	}
}
